...
- 2026-08-05: Added an initially expanded, per-card subtask visibility toggle to High Priority read-only cards; the accessible chevron stops click and keyboard events from opening the source task, while aggregate metadata and subtask count remain visible when minimized. Validation: `npm run build`, `git diff --check`, and editor diagnostics passed with only existing build warnings.
